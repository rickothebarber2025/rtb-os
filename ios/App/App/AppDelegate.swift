...
/// Capacitor's `packageClassList` auto-registration (generated by `cap sync`
/// from node_modules @capacitor/* plugins) has no way to discover a plugin
/// that lives directly in the app target, like GoogleHomeBridgePlugin below.
/// Registering it explicitly here is the supported way to add a native-only
/// plugin, and it survives every future `cap sync` run untouched.
class RTBBridgeViewController: CAPBridgeViewController {
    override func capacitorDidLoad() {
        super.capacitorDidLoad()
        bridge?.registerPluginInstance(GoogleHomeBridgePlugin())
    }
}
