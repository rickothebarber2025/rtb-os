...
const source = path.join(repoDir, 'integrations', 'arvis', 'workbench-overrides', 'ProcessViews.tsx');
const target = path.join(workbenchDir, 'src', 'components', 'ProcessViews.tsx');
...
if (!fs.existsSync(target)) {
  console.error(`[A.R.V.I.S.] Workbench ProcessViews target missing: ${target}`);
  process.exit(1);
}
...
console.log(`[A.R.V.I.S.] Applied RTB Workbench ProcessViews override to ${target}`);
