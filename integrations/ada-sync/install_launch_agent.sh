...
extract_existing_service_key() {
  [[ -f "$REPO_ENV" ]] || return 1
  local line
  line="$(grep -E '^(SUPABASE_SERVICE_ROLE_KEY|VITE_SUPABASE_SERVICE_ROLE_KEY)=' "$REPO_ENV" | head -n 1 || true)"
  [[ -n "$line" ]] || return 1
  printf '%s' "${line#*=}"
}

if [[ ! -f "$ENV_FILE" ]]; then
  existing_key="$(extract_existing_service_key || true)"
...
EOF
    chmod 600 "$ENV_FILE"
    echo "Created $ENV_FILE using the existing local RTB Supabase service key."
  else
    cat > "$ENV_FILE" <<'EOF'
SUPABASE_URL=https://qbeficojfoqgzjxrzxyg.supabase.co
...
EOF
    ...
    echo "No existing local service-role key was found. Add it locally, then run this installer again."
    exit 2
  fi
fi
...
# Test both localhost access and Supabase write before installing the recurring agent.
