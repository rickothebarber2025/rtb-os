...
export default function DashboardPage({
  ...
  const tabControlIds = useMemo(() => ({
    overview: [
      ...
      'dashboard-panel-overview-marketing',
  ...
  return (
    <div className="page-grid dashboard-page">
      ...
      <section
        {...panelAccessibility('overview', 'overview-workspace')}
        className={`panel full-span workspace-panel ${mobileGroupClass('overview')}`}
      >
      ...
      <section
        {...panelAccessibility('overview', 'overview-priority')}
        className={`panel full-span priority-board ${mobileGroupClass('overview')}`}
      >
        <div className="section-header">
          <div>
        ...
        <div className="priority-board__layout">
          <div className="priority-board__summary">
            ...
            <p>
              RTB OS highlights the work that needs follow-up first so you can act quickly instead of hunting through reports.
            </p>
            {actionSummary.total ? (
              <button className="primary-button" type="button" onClick={() => selectMobileTab('actions')}>
                Review open actions
              </button>
            ) : null}
          </div>
              ...
              topActions.map((item) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                  >
                    <div className="priority-board__item-icon"><Icon size={18} /></div>
                    <span><strong>{item.title}</strong><small>{item.detail}</small></span>
                  </button>
                );
              })
            ) : (
              <div className="empty-state compact"><h3>No urgent priorities</h3><p>This view looks clear right now.</p></div>
            )}
          </div>
        </div>
      </section>
      ...
      <section
        {...panelAccessibility('overview', 'overview-metrics')}
        className={`metrics-grid stat-strip ${mobileGroupClass('overview')}`}
      >
        ...
        <MetricCard
          ...
          value={payrollAllowed ? (latestRun ? formatCompactCurrency(latestRun.total_net_sales) : '$0') : formatCompactCurrency(appointmentMetrics.revenue)}
        />
      ...
      {instagramInsights ? (
        <section
          {...panelAccessibility('overview', 'overview-marketing')}
          className={`panel full-span ${mobileGroupClass('overview')}`}
        >
          <div className="section-header">
      ...
      <section
        {...panelAccessibility('overview', 'overview-profile')}
        className={`panel full-span ${mobileGroupClass('overview')}`}
      >
      ...
      <section
        {...panelAccessibility('overview', 'overview-checks')}
        className={`panel full-span ${mobileGroupClass('overview')}`}
      >
      ...
      <div
        {...panelAccessibility('overview', 'overview-checklists')}
        className={mobileGroupClass('overview')}
      >
        <AdminChecklistDashboard businessUnitId={allBusinessesView ? null : businessUnit?.id} />
      </div>

      <section
        {...panelAccessibility('actions', 'actions')}
        className={`panel full-span action-center-snapshot ${mobileGroupClass('actions')}`}
      >
        <div className="section-header">
      ...
      {hasAppointmentData ? (
        <section
          {...panelAccessibility('appointments', 'appointments')}
          className={`panel full-span dashboard-appointments ${mobileGroupClass('appointments')}`}
        >
          <div className="section-header">
            <div><span>Appointments</span><h2>Booking activity</h2></div>
            {appointmentSummary?.periodLabel ? <StatusBadge tone="muted">{appointmentSummary.periodLabel}</StatusBadge> : null}
          </div>
          <div className="metrics-grid dashboard-appointment-metrics">
            <MetricCard icon={CalendarDays} label="Bookings" trend="Appointment records" value={formatNumber(appointmentMetrics.bookings)} />
            <MetricCard icon={CircleDollarSign} label="Revenue" trend="Appointment revenue" value={formatCompactCurrency(appointmentMetrics.revenue)} />
            <MetricCard icon={Users} label="Clients" trend="Unique / recorded clients" value={formatNumber(appointmentMetrics.clients)} />
            <MetricCard icon={AlertCircle} label="No-shows" trend="Missed appointments" value={formatNumber(appointmentMetrics.noShows)} />
          </div>
        </section>
      ) : null}

      <section
        {...panelAccessibility('team', 'team-performance')}
        className={`panel ${mobileGroupClass('team')}`}
      >
        ...
        ) : <EmptyState icon={TrendingUp} title="No performance history" message="Performance history will appear here when records exist." />}
      </section>

      <section
        {...panelAccessibility('team', 'team-roster')}
        className={`panel ${mobileGroupClass('team')}`}
      >
        <div className="section-header"><div><span>Roster</span><h2>Commission profile</h2></div></div>
        <div className="stat-list">
          <div><span>Active staff</span><strong>{activeStaff.length}</strong></div>
      ...
      {payrollAllowed ? (
        <section
          {...panelAccessibility('finance', 'finance')}
          className={`panel two-thirds ${mobileGroupClass('finance')}`}
        >
          <div className="section-header"><div><span>Finance</span><h2>Recent payroll runs</h2></div><button className="ghost-button" type="button" onClick={() => setActivePage('payroll')}>Open payroll</button></div>
          {payrollRuns.length ? (
