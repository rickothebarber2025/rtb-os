...
drop trigger if exists finance_sync_square_sale on public.staff_daily_sales;
create trigger finance_sync_square_sale after insert or update of business_unit_id,sale_date,net_sales,square_team_member_id on public.staff_daily_sales for each row execute function private.sync_square_sale_to_finance();
...
insert into public.finance_transactions (business_unit_id,transaction_date,direction,amount,category,description,source,external_ref,notes)
select s.business_unit_id,s.sale_date,'income',coalesce(s.net_sales,0),'Sales revenue','Square sales'||case when s.square_team_member_id is not null then ' · team member '||s.square_team_member_id else '' end,'square','staff_daily_sales:'||s.id::text,'Automatically synchronized from RTB OS Square daily sales.'
from public.staff_daily_sales s where coalesce(s.net_sales,0)>0
